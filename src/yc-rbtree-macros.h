...
#ifndef __YC_RBTREE_MACROS_H_
#define __YC_RBTREE_MACROS_H_
...
#define YC_STD_GET_IS_RED(n)     n->is_red
#define YC_STD_SET_IS_RED(n,v)   n->is_red = v
 ...
 */
#define YC_RBTREE_INSERT(tree, node, collision_node)                         \
  YC_RBTREE_INSERT_(tree, node, collision_node)
#define YC_RBTREE_INSERT_AT(tree, parent, is_red, node)                      \
  YC_RBTREE_INSERT_AT_(tree, parent, is_right, node)
#define YC_RBTREE_REPLACE_NODE(tree, old, replacement)                       \
  YC_RBTREE_REPLACE_NODE_(tree, old, replacement)
#define YC_RBTREE_REMOVE(tree, node)                                         \
  YC_RBTREE_REMOVE_(tree, node)
#define YC_RBTREE_LOOKUP(tree, key, out)                                     \
  YC_RBTREE_LOOKUP_(tree, key, out)
#define YC_RBTREE_LOOKUP_COMPARATOR(tree, key, key_comparator, out)          \
  YC_RBTREE_LOOKUP_COMPARATOR_(tree, key, key_comparator, out)

#define YC_RBTREE_FIRST(tree, out)                                           \
  YC_RBTREE_FIRST_(tree, out)
#define YC_RBTREE_LAST(tree, out)                                            \
  YC_RBTREE_LAST_(tree, out)
#define YC_RBTREE_NEXT(tree, in, out)                                        \
  YC_RBTREE_NEXT_(tree, in, out)
#define YC_RBTREE_PREV(tree, in, out)                                        \
  YC_RBTREE_PREV_(tree, in, out)

#define YC_RBTREE_LOOKUP_OR_NEXT(tree, key, out)                                   \
  YC_RBTREE_LOOKUP_OR_NEXT_(tree, key, out)
#define YC_RBTREE_LOOKUP_OR_NEXT_COMPARATOR(tree, key, key_comparator, out)        \
  YC_RBTREE_LOOKUP_OR_NEXT_COMPARATOR_(tree, key, key_comparator, out)
#define YC_RBTREE_LOOKUP_OR_PREV(tree, key, out)                                    \
  YC_RBTREE_LOOKUP_OR_PREV_(tree, key, out)
#define YC_RBTREE_LOOKUP_OR_PREV_COMPARATOR(tree, key, key_comparator, out)         \
  YC_RBTREE_LOOKUP_OR_PREV_COMPARATOR_(tree, key, key_comparator, out)

#if 1
#undef YC_STMT_START
#define YC_STMT_START do
#undef YC_STMT_END
#define YC_STMT_END while(0)
#endif

#define YC_RBTREE_INSERT_(top,type,is_red,set_is_red,parent,left,right,comparator, node,collision_node) \
YC_STMT_START{                                                                 \
  type _dsk_last = NULL;                                                      \
  type _dsk_at = (top);                                                       \
                ...
                {                                                             \
                  if (_dsk_at == _dsk_at->parent->right)                      \
                    {                                                         \
                      ...
                      YC_RBTREE_ROTATE_LEFT (top,type,parent,left,right, _dsk_at);\
                    }                                                         \
                  set_is_red(_dsk_at->parent, 0);                             \
                  set_is_red(_dsk_at->parent->parent, 1);                     \
                  YC_RBTREE_ROTATE_RIGHT (top,type,parent,left,right,        \
            ...
            {                                                                 \
              ...
              if (_dsk_y != NULL && is_red (_dsk_y))                          \
                ...
                {                                                             \
                  ...
                  set_is_red(_dsk_at->parent, 0);                             \
                  set_is_red(_dsk_at->parent->parent, 1);                     \
                  YC_RBTREE_ROTATE_LEFT (top,type,parent,left,right,         \
                                          _dsk_at->parent->parent);           \
                }                                                             \
            }                                                                 \
        }                                                                     \
...
#define YC_RBTREE_INSERT_AT_(top,type,is_red,set_is_red,parent,left,right,comparator, parent_node,is_right,node) \
YC_STMT_START{                                                                 \
    {                                                                         \
      type _dsk_at;                                                           \
...
#define YC_RBTREE_REPLACE_NODE_(top,type,is_red,set_is_red,parent,left,right,comparator, old_node,replacement_node) \
YC_STMT_START{                                                               \
...
#define YC_RBTREE_REMOVE_(top,type,is_red,set_is_red,parent,left,right,comparator, node) \
/* Algorithms:273. */                                                         \
YC_STMT_START{                                                                 \
  ...
  if (_dsk_rb_del_z->left == NULL || _dsk_rb_del_z->right == NULL)            \
    ...
    {                                                                         \
      YC_RBTREE_NEXT_ (top,type,is_red,set_is_red,parent,left,right,comparator,\
...
#define YC_RBTREE_LOOKUP_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, \
...
#define YC_RBTREE_LOOKUP_OR_PREV_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, \
                                      key,key_comparator,out)                 \
YC_STMT_START{                                                                 \
  type _dsk_lookup_at = (top);                                                \
 ...
 */
#define YC_RBTREE_LOOKUP_OR_NEXT_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, \
                                      key,key_comparator,out)                 \
YC_STMT_START{                                                                 \
  type _dsk_lookup_at = (top);                                                \
  type _dsk_lookup_rv = NULL;                                                 \
...
#define YC_RBTREE_LOOKUP_(top,type,is_red,set_is_red,parent,left,right,comparator, key,out) \
  YC_RBTREE_LOOKUP_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, key,comparator,out)
#define YC_RBTREE_LOOKUP_OR_NEXT_(top,type,is_red,set_is_red,parent,left,right,comparator, key,out) \
  YC_RBTREE_LOOKUP_OR_NEXT_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, key,comparator,out)
#define YC_RBTREE_LOOKUP_OR_PREV_(top,type,is_red,set_is_red,parent,left,right,comparator, key,out) \
  YC_RBTREE_LOOKUP_OR_PREV_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, key,comparator,out)

/* these macros don't need the is_red/set_is_red macros, nor the comparator,
   so omit them, to keep the lines a bit shorter. */
#define YC_RBTREE_ROTATE_RIGHT(top,type,parent,left,right, node)             \
  YC_RBTREE_ROTATE_LEFT(top,type,parent,right,left, node)
#define YC_RBTREE_ROTATE_LEFT(top,type,parent,left,right, node)              \
YC_STMT_START{                                                                 \
...
/* iteration */
#define YC_RBTREE_NEXT_(top,type,is_red,set_is_red,parent,left,right,comparator, in, out)  \
YC_STMT_START{                                                                 \
...
#define YC_RBTREE_PREV_(top,type,is_red,set_is_red,parent,left,right,comparator, in, out)  \
  YC_RBTREE_NEXT_(top,type,is_red,set_is_red,parent,right,left,comparator, in, out)

#define YC_RBTREE_FIRST_(top,type,is_red,set_is_red,parent,left,right,comparator, out)  \
YC_STMT_START{                                                                 \
...
#define YC_RBTREE_LAST_(top,type,is_red,set_is_red,parent,left,right,comparator, out)  \
  YC_RBTREE_FIRST_(top,type,is_red,set_is_red,parent,right,left,comparator, out)

 /* --- RBC-Tree --- */
#define YC_RBCTREE_INSERT(tree, node, collision_node)                         \
  YC_RBCTREE_INSERT_(tree, node, collision_node)
#define YC_RBCTREE_REMOVE(tree, node)                                         \
  YC_RBCTREE_REMOVE_(tree, node)
#define YC_RBCTREE_LOOKUP(tree, key, out)                                     \
  YC_RBCTREE_LOOKUP_(tree, key, out)
#define YC_RBCTREE_LOOKUP_COMPARATOR(tree, key, key_comparator, out)          \
  YC_RBCTREE_LOOKUP_COMPARATOR_(tree, key, key_comparator, out)

#define YC_RBCTREE_FIRST(tree, out)                                           \
  YC_RBCTREE_FIRST_(tree, out)
#define YC_RBCTREE_LAST(tree, out)                                            \
  YC_RBCTREE_LAST_(tree, out)
#define YC_RBCTREE_NEXT(tree, in, out)                                        \
  YC_RBCTREE_NEXT_(tree, in, out)
#define YC_RBCTREE_PREV(tree, in, out)                                        \
  YC_RBCTREE_PREV_(tree, in, out)

#define YC_RBCTREE_LOOKUP_OR_NEXT(tree, key, out)                                   \
  YC_RBCTREE_LOOKUP_OR_NEXT_(tree, key, out)
#define YC_RBCTREE_LOOKUP_OR_NEXT_COMPARATOR(tree, key, key_comparator, out)        \
  YC_RBCTREE_LOOKUP_OR_NEXT_COMPARATOR_(tree, key, key_comparator, out)
#define YC_RBCTREE_LOOKUP_OR_PREV(tree, key, out)                                    \
  YC_RBCTREE_LOOKUP_OR_PREV_(tree, key, out)
#define YC_RBCTREE_LOOKUP_OR_PREV_COMPARATOR(tree, key, key_comparator, out)         \
  YC_RBCTREE_LOOKUP_OR_PREV_COMPARATOR_(tree, key, key_comparator, out)

#define YC_RBCTREE_GET_BY_INDEX(tree, index, out)                             \
  YC_RBCTREE_GET_BY_INDEX_(tree, index, out)
#define YC_RBCTREE_GET_BY_INDEX_UNCHECKED(tree, index, out)                   \
  YC_RBCTREE_GET_BY_INDEX_UNCHECKED_(tree, index, out)
#define YC_RBCTREE_GET_NODE_INDEX(tree, node, index_out)                      \
  YC_RBCTREE_GET_NODE_INDEX_(tree, node, index_out)
...
#define YC_RBCTREE_INSERT_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, node,collision_node) \
YC_STMT_START{                                                                 \
  type _dsk_last = NULL;                                                      \
  type _dsk_at = (top);                                                       \
  int _dsk_last_was_left = 0;                                        \
              ...
              if (_dsk_y != NULL && is_red (_dsk_y))                          \
                ...
                {                                                             \
                  if (_dsk_at == _dsk_at->parent->right)                      \
                    {                                                         \
                      _dsk_at = _dsk_at->parent;                              \
                      YC_RBCTREE_ROTATE_LEFT (top,type,parent,left,right,get_count,set_count, _dsk_at);\
                    }                                                         \
                  set_is_red(_dsk_at->parent, 0);                             \
                  set_is_red(_dsk_at->parent->parent, 1);                     \
                  YC_RBCTREE_ROTATE_RIGHT (top,type,parent,left,right,get_count,set_count,\
                                         _dsk_at->parent->parent);            \
                }                                                             \
            }                                                                 \
          else                                                                \
            {                                                                 \
              ...
              if (_dsk_y != NULL && is_red (_dsk_y))                          \
                ...
                {                                                             \
                  ...
                  set_is_red(_dsk_at->parent, 0);                             \
                  set_is_red(_dsk_at->parent->parent, 1);                     \
                  YC_RBCTREE_ROTATE_LEFT (top,type,parent,left,right,get_count,set_count,\
                                          _dsk_at->parent->parent);           \
                }                                                             \
            }                                                                 \
        }                                                                     \
...
#define YC_RBCTREE_REMOVE_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, node) \
/* Algorithms:273. */                                                         \
YC_STMT_START{                                                                 \
  type _dsk_rb_del_z = (node);                                                \
  type _dsk_rb_del_x;                                                         \
  ...
  if (!_dsk_rb_del_y->parent)                                                 \
	...
	_dsk_rb_del_y->parent->right = _dsk_rb_del_x;                         \
      _YC_RBCTREE_FIX_COUNT_AND_UP(type,parent,left,right,get_count,set_count, _dsk_rb_del_y->parent);\
    }                                                                         \
  _dsk_rb_del_fixup = !is_red(_dsk_rb_del_y);                                 \
  if (_dsk_rb_del_y != _dsk_rb_del_z)                                         \
    {                                                                         \
	...
	{                                                                     \
      ...
      _YC_RBCTREE_FIX_COUNT_AND_UP(type,parent,left,right,get_count,set_count, _dsk_rb_del_y);\
                                                                              \
      if (_dsk_rb_del_y->left)                                                \
	_dsk_rb_del_y->left->parent = _dsk_rb_del_y;                          \
      if (_dsk_rb_del_y->right)                                               \
...
#define YC_RBCTREE_LOOKUP_COMPARATOR_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, \
                                      key,key_comparator,out)                 \
 YC_RBTREE_LOOKUP_COMPARATOR_(top,type,is_red,set_count,parent,left,right,comparator, key,key_comparator,out)
#define YC_RBCTREE_LOOKUP_OR_PREV_COMPARATOR_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, \
                                      key,key_comparator,out)                 \
 YC_RBTREE_LOOKUP_OR_PREV_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, key,key_comparator,out)
#define YC_RBCTREE_LOOKUP_OR_NEXT_COMPARATOR_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, \
                                      key,key_comparator,out)                 \
 YC_RBTREE_LOOKUP_OR_NEXT_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, key,key_comparator,out)
#define YC_RBCTREE_LOOKUP_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, key,out) \
  YC_RBTREE_LOOKUP_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, key,comparator,out)
#define YC_RBCTREE_LOOKUP_OR_NEXT_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, key,out) \
  YC_RBTREE_LOOKUP_OR_NEXT_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, key,comparator,out)
#define YC_RBCTREE_LOOKUP_OR_PREV_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, key,out) \
  YC_RBTREE_LOOKUP_OR_PREV_COMPARATOR_(top,type,is_red,set_is_red,parent,left,right,comparator, key,comparator,out)
#define YC_RBCTREE_NEXT_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, in, out)  \
   YC_RBTREE_NEXT_(top,type,is_red,set_is_red,parent,left,right,comparator, in, out)
#define YC_RBCTREE_PREV_(top,type,is_red,set_is_red,parent,left,right,comparator, in, out)  \
   YC_RBTREE_PREV_(top,type,is_red,set_is_red,parent,left,right,comparator, in, out)
#define YC_RBCTREE_FIRST_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, out)  \
  YC_RBTREE_FIRST_(top,type,is_red,set_is_red,parent,left,right,comparator, out)
#define YC_RBCTREE_LAST_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, out)  \
  YC_RBTREE_LAST_(top,type,is_red,set_is_red,parent,left,right,comparator, out)

#define YC_RBCTREE_GET_BY_INDEX_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, index,out) \
  YC_STMT_START{                                                                \
    if (top == NULL || (index) >= get_count(top))                              \
      out = NULL;                                                              \
    else                                                                       \
      YC_RBCTREE_GET_BY_INDEX_UNCHECKED_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, index,out);\
  }YC_STMT_END
#define YC_RBCTREE_GET_BY_INDEX_UNCHECKED_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, index,out) \
   YC_STMT_START{                                                               \
     type _dsk_at = (top);                                                     \
...
#define YC_RBCTREE_GET_NODE_INDEX_(top,type,is_red,set_is_red,get_count,set_count,parent,left,right,comparator, node,index_out) \
   YC_STMT_START{                                                             \
     type _dsk_at = (node);                                                    \
...
#define YC_RBCTREE_ROTATE_RIGHT(top,type,parent,left,right,get_count,set_count, node) \
  YC_RBCTREE_ROTATE_LEFT(top,type,parent,right,left,get_count,set_count, node)
#define YC_RBCTREE_ROTATE_LEFT(top,type,parent,left,right,get_count,set_count, node)  \
YC_STMT_START{                                                                 \
  type _dsk_rot_x = (node);                                                   \
  type _dsk_rot_y = _dsk_rot_x->right;                                        \
                                                                              \
...
#define _YC_RBCTREE_FIX_COUNT(type,parent,left,right,get_count,set_count, node)          \
YC_STMT_START{                                                                 \
...
#define _YC_RBCTREE_FIX_COUNT_AND_UP(type,parent,left,right,get_count,set_count, node)   \
YC_STMT_START{                                                                 \
  type _tmp_fix_count_up;                                                     \
  for (_tmp_fix_count_up = (node);                                            \
       _tmp_fix_count_up != NULL;                                             \
       _tmp_fix_count_up = _tmp_fix_count_up->parent)                         \
    _YC_RBCTREE_FIX_COUNT (type,parent,left,right,get_count,set_count, _tmp_fix_count_up);\
}YC_STMT_END
